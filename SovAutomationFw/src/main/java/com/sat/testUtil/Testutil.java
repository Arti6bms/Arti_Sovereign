...
public class Testutil {
	...
	public void sendKeys(WebDriver driver, WebElement element, int timeout, String value) {
		try {
			...
			System.out.println("until, Exception Error message : " + e);
		}
	}

	public void inputText(WebDriver driver, String fieldPropertyNameName, int timeout, String value) {
		try {
			System.out.println("entering value in - " + fieldPropertyNameName);
			JavascriptExecutor js = (JavascriptExecutor) driver;
			Thread.sleep(3000);
			// Scrolling down the page till the element is found
			js.executeScript("arguments[0].scrollIntoView();",
					driver.findElement(By.xpath("//input[contains(@aria-label,'" + fieldPropertyNameName + "')]")));
			...
			System.out.println("user entered value in -" + fieldPropertyNameName);
		} catch (Exception e) {
			System.out.println("until, Exception Error message : " + e);
		}
	}
	...
	public void clickOn(WebDriver driver1, WebElement element, int timeout) {
		...
		System.out.println("user click on element : " + element);
	...
	public void lockedUnlocked(DataTable datatable) {
		...
		try {
			for (int j = 1; j <= datatable.asList().size(); j++) {
				...
				System.out.println("actual.get(0).get(j) = " + actual.get(0).get(j));
